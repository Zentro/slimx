 ...
 */
pub fn sign(sk: [u8; 32], msg: Vec<u8>, nonce: Vec<u8>) -> [u8; 64] {
    ...
    let (ed_pk, ed_sk) = calculate_key_pair(sk_scalar);
    
    let concat1 = [&ed_sk, msg.as_slice(), nonce.as_slice()].concat();
    let r0 = hash_i(1, concat1);
 ...
 */
pub fn verify(pk: [u8; 32], msg: Vec<u8>, sig: [u8; 64]) -> bool {
 ...
 */
fn hash_i(i: u128, msg: Vec<u8>) -> [u8; 64]{
...
fn calculate_key_pair(scal: Scalar) -> ([u8; 32], [u8; 32]) {
